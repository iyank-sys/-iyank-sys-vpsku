...
data_ip="https://raw.githubusercontent.com/iyank-sys/-iyank-sys-vpsku/main/ip "
checking_sc() {
  ...
  if [[ $date_list < $useexp ]]; then
    ...
    echo -e "\033[42m          MANDIRI CELL AUTOSCRIPT          \033[0m"
    echo -e "\033[1;93m────────────────────────────────────────────\033[0m"
    echo -e ""
    ...
    echo -e "      \033[0;36mTelegram${NC} t.me/iyank"
    echo -e "      ${GREEN}WhatsApp${NC} wa.me/628974304684"
    echo -e "\033[1;93m────────────────────────────────────────────\033[0m"
    exit
  fi
}
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/iyank-sys/-iyank-sys-vpsku/main/ip  | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/iyank-sys/-iyank-sys-vpsku/main/ip  | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/iyank-sys/-iyank-sys-vpsku/main/ip  | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/iyank-sys/-iyank-sys-vpsku/main/"
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/iyank-sys/-iyank-sys-vpsku/main/ip  | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/iyank-sys/-iyank-sys-vpsku/main/ip  | grep $ipsaya | awk '{print $3}')
...
function menu(){
    ...
    wget -q https://raw.githubusercontent.com/iyank-sys/-iyank-sys-vpsku/main/kikitolan/mukung/menu.zip
    unzip menu.zip
    chmod +x menu/*
...
clear
    ...
    wget -q https://raw.githubusercontent.com/iyank-sys/-iyank-sys-vpsku/main/udp-custom.sh &&  chmod +x udp-custom.sh && ./udp-custom.sh
}
